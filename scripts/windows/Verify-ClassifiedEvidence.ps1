...
Require ($e.Outcome -eq 'PASS_CLASSIFIED' -and $e.Classified -and $e.CompletedUtc -and $e.HostExitCode -eq 0 -and $e.PatternExitCode -eq 0 -and !$e.PatternForcedStop -and !$e.HostForcedStop -and !$e.CleanupCheckError) 'Classified execution/clean shutdown did not pass'
...
foreach($session in $sessions){
 ...
 Require ($c.D -eq 0 -and $c.E -eq 0 -and $c.A+$c.B+$c.C -eq $r.received) 'Classification totals'
 ...
 $counts=@{1=0;2=0;3=0};$transitions=0;$lastClass=0
 for($i=0;$i -lt $frames.Count;$i++){
  ...
  Require ($cl.frame_id -eq $f.frame_id -and $cl.source_qpc -eq $f.source_qpc -and $cl.nonce -eq $f.nonce -and $cl.counter -eq $f.counter -and [uint32]$cl.previous_counter -eq $highCounter -and $k -in @(1,2,3)) 'Classification/frame association'
  if($k -eq 1 -or $k -eq 3){$counter=[uint32]$f.counter;Require ([uint32]$f.nonce -eq $nonce -and $counter -gt 0 -and $ledger.ContainsKey($counter) -and $ledger[$counter] -eq [uint64]$cl.render_qpc -and $ledger[$counter] -le $qpc -and $counter -le [uint32]$cl.published) 'Pattern render ledger';if($k -eq 1){Require ($counter -ge $highCounter) 'Uncorroborated regression'}else{Require ($counter -lt $highCounter) 'Stale classification without regression'};$highCounter=[Math]::Max($highCounter,$counter)}
 ...
 Require ($counts[1] -eq $c.A -and $counts[2] -eq $c.B -and $counts[3] -eq $c.C -and $gaps -eq $r.id_gaps) 'Summary/class/drop counts'
...
$summary=@{Outcome='PASS_CLASSIFIED';HistoricalSoak='UNKNOWN';Runs=$reports;Resources=$profiles;ResourceTrendRequiresReview=$true;Scope='Sampled-region content corroboration and protocol/resource invariants; not whole-frame pixel proof'}
