# PHASE 3C integration regression check, derived from unchanged PHASE 3B verifier.
...
if($dir -notmatch '[\\/]phase3c[\\/]'){throw 'PHASE 3C evidence directory required'}
...
& (Join-Path $PSScriptRoot 'Verify-TransportHandoff.ps1') -EvidenceDirectory $dir -MinimumSeconds $MinimumSeconds -RequireReconnect:$RequireReconnect -ReportPath (Join-Path $dir 'handoff-verification.json') | Out-Null
if(Test-Path (Join-Path $dir 'encoding-verification.json')){throw 'Preserve existing encoding verification'}
