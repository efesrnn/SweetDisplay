...
$output = Join-Path $outputDirectory 'sweetdisplay-diag-v11'
...
$arguments = @(
    ...
    '-DSWEETDISPLAY_DIAGNOSTIC_V11=1',
...
& $compiler '-std=c++17' '-Wall' '-Wextra' '-Werror' '-fsyntax-only' (Join-Path $repo 'device\final\diagnostic\tests\flip_events_test.cpp')
if ($LASTEXITCODE -ne 0) { throw 'V11 flip-event compile-time tests failed' }
Write-Output 'FLIP_EVENT_TESTS=PASS (18 compile-time cases)'

& $compiler @arguments
if ($LASTEXITCODE -ne 0) { throw "Diagnostic v11 compile failed: $LASTEXITCODE" }
& $strip --strip-all $output
if ($LASTEXITCODE -ne 0) { throw "Diagnostic v11 strip failed: $LASTEXITCODE" }

$elfReport = & $readelf -h -l -d $output 2>&1
if ($LASTEXITCODE -ne 0) { throw "Diagnostic v11 ELF inspection failed: $LASTEXITCODE" }
if (($elfReport -join "`n") -match 'INTERP|Dynamic section at offset') {
    throw 'Diagnostic v11 unexpectedly has an interpreter or dynamic section'
}
