...
$privateRoot = Join-Path $repo 'docs\evidence\private\final-boot1a\v2'
...
$diagnostic = Join-Path $repo 'out\final-device\Release\sweetdisplay-diag-v2'
$ramdisk = Join-Path $privateRoot 'SWEETDISPLAY-TEMP-BOOT-v2.ramdisk.gz'
$manifest = Join-Path $privateRoot 'boot-critical-manifest.json'
$candidate = Join-Path $privateRoot 'SWEETDISPLAY-TEMP-BOOT-v2.img'
...
$initRc = Join-Path $repo 'device\final\diagnostic\init.sweetdisplay.v2.rc'
...
# Omit --ueventd-rc deliberately: v2 retains the exact known-bootable stock
# recovery ueventd rules, including pmsg, DRM and input node permissions.
& $Python $ramdiskBuilder `
    ...
    --output $ramdisk `
    --manifest $manifest
if ($LASTEXITCODE -ne 0) { throw "Ramdisk v2 build failed: $LASTEXITCODE" }
...
if ($LASTEXITCODE -ne 0) { throw "Boot image v2 build failed: $LASTEXITCODE" }

& $Python $unpackBootimg --boot_img $candidate --out $validated --format info
if ($LASTEXITCODE -ne 0) { throw "Candidate v2 re-unpack failed: $LASTEXITCODE" }
...
if ($validatedKernelHash -ne $stockKernelHash) { throw 'Candidate v2 kernel hash mismatch' }
if ($validatedDtbHash -ne $stockDtbHash) { throw 'Candidate v2 DTB hash mismatch' }
if ($validatedRamdiskHash -ne $ramdiskHash) { throw 'Candidate v2 ramdisk hash mismatch' }
if ($item.Length -gt 134217728 -or $item.Length -gt 805306368) {
    throw 'Candidate v2 exceeds a validated size envelope'
}
...
if ($magic -ne 'ANDROID!' -or $pageSize -ne 4096 -or
    ...
    throw 'Candidate v2 boot header mismatch'
}
if ($footerMagic -eq 'AVBf') { throw 'Unexpected AVB footer on candidate v2' }
