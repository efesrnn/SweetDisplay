...
if($ReviewDirectory){
 ...
 if($review -notmatch '[\\/]phase3c[\\/]' -or $review -eq $dir){throw 'Separate private PHASE3C review directory required'}
}
...
if($Reconnect){
 $e=Json (Join-Path $dir 'execution.json');Require ($e.ReceiverKilledAsPlanned -and $e.ReceiverRestarted -and $sessionFirst.Count -ge 2 -and $t.disconnected -gt 0 -and $t.resync_skipped -gt 0) 'Real disconnect/recovery evidence'
 $termination=@($e.Commands|Where-Object Command -eq 'Terminate own active receiver');Require ($termination.Count -eq 1 -and $termination[0].Result.HostStillRunning) 'Host remained active at receiver termination'
