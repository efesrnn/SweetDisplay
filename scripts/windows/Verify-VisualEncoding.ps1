...
# PHASE 3D integration regression check, derived from unchanged PHASE 3B verifier.
param([Parameter(Mandatory=$true)][string]$EvidenceDirectory,[int]$MinimumSeconds=20,[switch]$RequireReconnect,[string]$DecodeSubdirectory='',[string]$ReviewDirectory)
$ErrorActionPreference='Stop'
$dir=(Resolve-Path -LiteralPath $EvidenceDirectory).Path
if($dir -notmatch '[\\/]phase3d[\\/]'){throw 'PHASE 3D evidence directory required'}
$review=if($ReviewDirectory){(Resolve-Path -LiteralPath $ReviewDirectory).Path}else{$dir}
if($ReviewDirectory -and ($review -notmatch '[\\/]phase3d[\\/]' -or $review -eq $dir)){throw 'Separate private PHASE3D review directory required'}
...
& (Join-Path $PSScriptRoot 'Verify-VisualHandoff.ps1') -EvidenceDirectory $dir -MinimumSeconds $MinimumSeconds -RequireReconnect:$RequireReconnect -ReportPath (Join-Path $review 'handoff-verification.json') | Out-Null
if(Test-Path (Join-Path $review 'encoding-verification.json')){throw 'Preserve existing encoding verification'}
$sessions=@('host');if($RequireReconnect){$sessions+='reconnect'}
$summary=@()
foreach($session in $sessions){
 $p=Join-Path $dir $session
 $decodePath=if($ReviewDirectory){Join-Path $review ($session+'-decode')}elseif($DecodeSubdirectory){Join-Path $p $DecodeSubdirectory}else{$p}
...
$report|ConvertTo-Json -Depth 8|Set-Content (Join-Path $review 'encoding-verification.json') -Encoding utf8
