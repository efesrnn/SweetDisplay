...
param([Parameter(Mandatory=$true)][string]$RunName,
 ...
 [switch]$Classified,[switch]$Reconnect)
...
$dir=Join-Path $repo ('docs/evidence/private/phase3a1/'+$RunName)
...
try {
 ...
 $patternArgs='"'+$dir+'" '+$nonce+' '+($Seconds+60)+' 0 1 '+$ControlCase
 ...
 $start=[datetime]::UtcNow;$deadline=$start.AddSeconds($Seconds+15);$nextProfile=$start;$nextHealth=$start.AddSeconds(30);$healthIndex=0
