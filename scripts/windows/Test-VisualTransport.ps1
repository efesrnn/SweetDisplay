...
param([Parameter(Mandatory=$true)][string]$RunName,
 [ValidateRange(20,120)][int]$Seconds=30,
 [ValidateSet('observe','cover','snapshot','minimize','foreground')][string]$ControlCase='observe',
 [switch]$Classified=$true,[switch]$Reconnect,[int]$Width=2400,[int]$Height=1080,[int]$Rate=60,[int]$Bitrate=30000000,[switch]$UncappedSubmission=$true,
 [switch]$DisconnectReceiver,[ValidateRange(1024,65535)][int]$Port=48231,[switch]$StageDiagnostics,[switch]$AsyncDiagnostics)
...
if($Reconnect -or !$Classified -or !$UncappedSubmission){throw 'PHASE3D requires classified uncapped single Host session'}
...
$result=@{Outcome='RUNNING';ControlCase=$ControlCase;Classified=[bool]$Classified;RequestedSeconds=$Seconds;StartedUtc=[datetime]::UtcNow.ToString('o');Commands=@();Error=$null;StageDiagnostics=[bool]($StageDiagnostics -or $AsyncDiagnostics);AsyncDiagnostics=[bool]$AsyncDiagnostics;FinalAcceptance=$false}
...
public static class SweetDisplayDiagNative {
 ...
 [DllImport("kernel32.dll",SetLastError=true)] public static extern bool GetFileSizeEx(IntPtr h,out long n);
...
$pattern=$null;$consumer=$null;$before=$null;$driverPid=0;$receiver=$null;$receiverHandle=[IntPtr]::Zero;$receiverKilled=$false;$receiverRestarted=$false;$receiverDir=$null
$encodeArgs+=' --transport-port '+$Port
function StartReceiver([int]$index){
 $script:receiverDir=Join-Path $dir ('receiver-'+$index);New-Item -ItemType Directory $receiverDir|Out-Null
 $capture=if($DisconnectReceiver -and $index -eq 1){0}else{1}
 $args='--listen "'+$receiverDir+'" '+$Port+' '+($Seconds+40)+' '+$capture
 ...
 $executable=Join-Path $repo ('out/'+$variant+'/SweetDisplayDeviceSimulator.exe')
 if($StageDiagnostics -or $AsyncDiagnostics){Copy-Item -LiteralPath $executable -Destination (Join-Path $receiverDir SweetDisplayDeviceSimulator.exe);Get-FileHash -LiteralPath $executable -Algorithm SHA256|ConvertTo-Json|Set-Content (Join-Path $receiverDir binary-sha256.json)}
 $script:receiver=Start-Process $executable -ArgumentList $args -WindowStyle Normal -PassThru -RedirectStandardOutput (Join-Path $receiverDir 'stdout.txt') -RedirectStandardError (Join-Path $receiverDir 'stderr.txt')
 $script:receiverHandle=$receiver.Handle;Log 'Start transport endpoint' @{ProcessId=$receiver.Id;Arguments=$args;Index=$index}
}
function LiveSize($path){$h=[SweetDisplayDiagNative]::CreateFileW($path,0,7,[IntPtr]::Zero,3,0,[IntPtr]::Zero);if($h -eq [IntPtr]::new(-1)){throw 'Receiver evidence metadata unavailable'};try{[long]$size=0;if(![SweetDisplayDiagNative]::GetFileSizeEx($h,[ref]$size)){throw 'Receiver evidence size query'};return $size}finally{[SweetDisplayDiagNative]::CloseHandle($h)|Out-Null}}
try {
 ...
 $helpers=@(Get-Process SweetDisplayDevice -ErrorAction Stop);if($helpers.Count -ne 1){throw 'Unique existing helper required'};$result.HelperPid=$helpers[0].Id
 StartReceiver 1
 ...
 $hostExe=Join-Path $repo 'out/transport-host-content-v2/SweetDisplayHost.exe'
 ...
 while(!$consumer.WaitForExit(100)){
  ...
  if($DisconnectReceiver -and !$receiverKilled -and $elapsed -ge 12){
   if($receiver.HasExited -or (LiveSize (Join-Path $receiverDir 'received.csv')) -lt 1024){throw 'Receiver was not actively validating frames before termination'}
   if(!$receiver.CloseMainWindow()){throw 'Own simulator WM_CLOSE request failed'};if(!$receiver.WaitForExit(5000)){throw 'Own visual receiver close timeout'}
   [uint32]$rc=0;if(![SweetDisplayDiagNative]::GetExitCodeProcess($receiverHandle,[ref]$rc)){throw 'Receiver exit query'}
   if($rc -ne 0){throw 'Controlled receiver close failed'};$receiverKilled=$true;$restartAt=([datetime]::UtcNow).AddSeconds(3);Log 'Close own active visual receiver' @{ProcessId=$receiver.Id;ExitCode=$rc;HostStillRunning=(!$consumer.HasExited)}
   Health 'receiver-absent'|Out-Null
  }
  if($receiverKilled -and !$receiverRestarted -and [datetime]::UtcNow -ge $restartAt){StartReceiver 2;$receiverRestarted=$true}
  if((!$receiverKilled -or $receiverRestarted) -and $receiver.HasExited){
   [uint32]$receiverDone=0
   if(![SweetDisplayDiagNative]::GetExitCodeProcess($receiverHandle,[ref]$receiverDone)){throw 'Receiver exit query during drain'}
   $completion=Wait-TransportReceiverDrain -HostProcess $consumer -ReceiverExitCode $receiverDone -ReceiverDirectory $receiverDir
   Log 'Receiver DRAIN_ACK completed; bounded wait for Host exit' $completion
   break # The normal native Host exit-code check below is still mandatory.
  }
  ...
  if([datetime]::UtcNow -ge $nextProfile){
   foreach($entry in @(@{Role='Host';Id=$consumer.Id},@{Role='Pattern';Id=$pattern.Id},@{Role='Driver';Id=$driverPid},@{Role='Receiver';Id=$receiver.Id})){
  ...
  try{
   Get-CimInstance -ClassName Win32_PerfFormattedData_GPUPerformanceCounters_GPUEngine -Filter ("Name LIKE '%pid_"+$consumer.Id+"_%' OR Name LIKE '%pid_"+$receiver.Id+"_%'") | Select-Object @{Name='ElapsedSeconds';Expression={([datetime]::UtcNow-$start).TotalSeconds}},Name,UtilizationPercentage | Export-Csv (Join-Path $dir 'gpu-engines.csv') -Append -NoTypeInformation -Encoding utf8
 ...
 $result.Outcome=if($Classified){'PASS_TRANSPORT_SOURCE'}else{'NO_ANOMALY_REPRODUCED'}
 ...
 if($receiver){
  if(!$receiver.WaitForExit(10000)){$receiver.Kill();$result.ReceiverForcedStop=$true;$result.Outcome='ERROR';$result.CleanupCheckError='Receiver drain timeout'}
  [uint32]$receiverCode=0;if(![SweetDisplayDiagNative]::GetExitCodeProcess($receiverHandle,[ref]$receiverCode)){$result.Outcome='ERROR';$result.CleanupCheckError='Receiver exit query'}
  $result.ReceiverExitCode=$receiverCode;$result.ReceiverClosedAsPlanned=$receiverKilled;$result.ReceiverRestarted=$receiverRestarted
  if($receiverCode -ne 0){$result.Outcome='ERROR';$result.CleanupCheckError='Receiver failed; inspect stderr'}
 }
 ...
 try{
  ...
  if($result.HelperPid -and !(Get-Process -Id $result.HelperPid -ErrorAction SilentlyContinue)){throw 'Original helper unavailable'}
  $from=([datetime]$result.StartedUtc).ToLocalTime()
  $events=@(Events @{LogName='Application';StartTime=$from;Id=1000,1001}|Where-Object {$_.Message -match 'WUDFHost|SweetDisplayDriver|SweetDisplayGpuPattern|SweetDisplayHost|SweetDisplayDeviceSimulator'})
...
if($result.Outcome -notin @('NO_ANOMALY_REPRODUCED','PASS_TRANSPORT_SOURCE')){throw ($result.Outcome+': '+$result.Error)}
