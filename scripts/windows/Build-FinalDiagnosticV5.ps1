...
$output = Join-Path $outputDirectory 'sweetdisplay-diag-v5'
...
& $compiler @arguments
if ($LASTEXITCODE -ne 0) { throw "Diagnostic v5 compile failed: $LASTEXITCODE" }

& $strip --strip-all $output
if ($LASTEXITCODE -ne 0) { throw "Diagnostic v5 strip failed: $LASTEXITCODE" }

$elfReport = & $readelf -h -l -d $output 2>&1
if ($LASTEXITCODE -ne 0) { throw "Diagnostic v5 ELF inspection failed: $LASTEXITCODE" }
if (($elfReport -join "`n") -match 'INTERP|Dynamic section at offset') {
    throw 'Diagnostic v5 unexpectedly has an interpreter or dynamic section'
}
