...
$privateRoot = Join-Path $repo 'docs\evidence\private\final-boot-bringup\v3'
...
$diagnostic = Join-Path $repo 'out\final-device\Release\sweetdisplay-diag-v3'
$ramdisk = Join-Path $privateRoot 'SWEETDISPLAY-TEMP-BOOT-v3.ramdisk.gz'
$manifest = Join-Path $privateRoot 'boot-critical-manifest.json'
$candidate = Join-Path $privateRoot 'SWEETDISPLAY-TEMP-BOOT-v3.img'
...
$initRc = Join-Path $repo 'device\final\diagnostic\init.sweetdisplay.v3.rc'
...
& $Python $ramdiskBuilder `
    ...
    --output $ramdisk `
    --manifest $manifest
if ($LASTEXITCODE -ne 0) { throw "Ramdisk v3 build failed: $LASTEXITCODE" }
...
if ($LASTEXITCODE -ne 0) { throw "Boot image v3 build failed: $LASTEXITCODE" }

& $Python $unpackBootimg --boot_img $candidate --out $validated --format info
if ($LASTEXITCODE -ne 0) { throw "Candidate v3 re-unpack failed: $LASTEXITCODE" }
...
if ($validatedKernelHash -ne $stockKernelHash) { throw 'Candidate v3 kernel hash mismatch' }
if ($validatedDtbHash -ne $stockDtbHash) { throw 'Candidate v3 DTB hash mismatch' }
if ($validatedRamdiskHash -ne $ramdiskHash) { throw 'Candidate v3 ramdisk hash mismatch' }
if ($item.Length -gt 134217728 -or $item.Length -gt 805306368) {
    throw 'Candidate v3 exceeds a validated size envelope'
}
...
if ($magic -ne 'ANDROID!' -or $pageSize -ne 4096 -or
    ...
    throw 'Candidate v3 boot header mismatch'
}
if ($footerMagic -eq 'AVBf') { throw 'Unexpected AVB footer on candidate v3' }

$manifestData = Get-Content -Raw -LiteralPath $manifest | ConvertFrom-Json
foreach ($required in @('mnt', 'debug_ramdisk')) {
