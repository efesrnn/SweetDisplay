...
param([Parameter(Mandatory=$true)][string]$RunName,
 ...
 [switch]$DisconnectReceiver,[ValidateRange(1024,65535)][int]$Port=48231,
 [ValidateSet('content-v2','perf3')][string]$BuildFlavor='content-v2')
...
if($Reconnect -or !$Classified -or !$UncappedSubmission){throw 'PHASE3C requires classified uncapped single Host session'}
...
function StartReceiver([int]$index){
 ...
 $args='"'+$receiverDir+'" '+$Port+' '+($Seconds+40)+' 0 '+$capture
 $script:receiver=Start-Process (Join-Path $repo ('out/device-simulator-'+$BuildFlavor+'/SweetDisplayDeviceSimulator.exe')) -ArgumentList $args -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $receiverDir 'stdout.txt') -RedirectStandardError (Join-Path $receiverDir 'stderr.txt')
...
try {
 ...
 $hostExe=Join-Path $repo ('out/transport-host-'+$BuildFlavor+'/SweetDisplayHost.exe')
 ...
 while(!$consumer.WaitForExit(100)){
  ...
  if($DisconnectReceiver -and !$receiverKilled -and $elapsed -ge 12){
   ...
   $receiver.Kill();if(!$receiver.WaitForExit(5000)){throw 'Own receiver termination timeout'}
   [uint32]$rc=0;if(![SweetDisplayDiagNative]::GetExitCodeProcess($receiverHandle,[ref]$rc)){throw 'Receiver exit query'}
   $receiverKilled=$true;$restartAt=([datetime]::UtcNow).AddSeconds(3);Log 'Terminate own active receiver' @{ProcessId=$receiver.Id;ExitCode=$rc;HostStillRunning=(!$consumer.HasExited)}
 ...
 if($receiver){
  ...
  $result.ReceiverExitCode=$receiverCode;$result.ReceiverKilledAsPlanned=$receiverKilled;$result.ReceiverRestarted=$receiverRestarted
