...
static inline void FillSignalInfo(DISPLAYCONFIG_VIDEO_SIGNAL_INFO& Mode, DWORD Width, DWORD Height, DWORD VSync, bool bMonitorMode)
{
    ...
    Mode.totalSize.cx = Width + SweetDisplay::HorizontalBlank;
    Mode.activeSize.cy = Height;
    Mode.totalSize.cy = Height + SweetDisplay::VerticalBlank;
    ...
    Mode.hSyncFreq.Numerator = VSync * Mode.totalSize.cy;
    Mode.hSyncFreq.Denominator = 1;

    Mode.scanLineOrdering = DISPLAYCONFIG_SCANLINE_ORDERING_PROGRESSIVE;

    Mode.pixelRate = ((UINT64) VSync) * ((UINT64) Mode.totalSize.cx) * ((UINT64) Mode.totalSize.cy);
}
...
void IndirectDeviceContext::InitAdapter()
{
    ...
    AdapterCaps.EndPointDiagnostics.pEndPointFriendlyName = SweetDisplay::Name;
    AdapterCaps.EndPointDiagnostics.pEndPointManufacturerName = L"SweetDisplay Project";
    AdapterCaps.EndPointDiagnostics.pEndPointModelName = L"AMOLED virtual prototype";
...
void IndirectDeviceContext::FinishInit(UINT ConnectorIndex)
{
    ...
    MonitorInfo.MonitorDescription.DataSize = sizeof(SweetDisplay::Edid);
    MonitorInfo.MonitorDescription.pData = const_cast<BYTE*>(SweetDisplay::Edid);

    // Stable identity for this single virtual prototype, never a phone serial.
    MonitorInfo.MonitorContainerId = {0x6c5b17ad, 0x6730, 0x4d95, {0x95, 0x0b, 0x5c, 0xce, 0x15, 0xb8, 0x86, 0x71}};
...
static NTSTATUS ModeBufferResult(SweetDisplay::BufferStatus Status)
{
    switch (Status)
    {
    case SweetDisplay::BufferStatus::TooSmall: return STATUS_BUFFER_TOO_SMALL;
    case SweetDisplay::BufferStatus::Invalid: return STATUS_INVALID_PARAMETER;
...
{
    pOutArgs->MonitorModeBufferOutputCount = SweetDisplay::ModeCount;
    pOutArgs->PreferredMonitorModeIdx = 0;
    if (pInArgs->MonitorDescription.Type != IDDCX_MONITOR_DESCRIPTION_TYPE_EDID ||
        pInArgs->MonitorDescription.DataSize != sizeof(SweetDisplay::Edid) ||
        pInArgs->MonitorDescription.pData == nullptr ||
        memcmp(pInArgs->MonitorDescription.pData, SweetDisplay::Edid, sizeof(SweetDisplay::Edid)) != 0)
        return STATUS_INVALID_PARAMETER;

    const auto status = SweetDisplay::CheckBuffer(pInArgs->MonitorModeBufferInputCount, pInArgs->pMonitorModes);
    if (status != SweetDisplay::BufferStatus::Fill) return ModeBufferResult(status);
    for (DWORD i = 0; i < SweetDisplay::ModeCount; ++i)
    {
        const auto& m = SweetDisplay::Modes[i];
        // Only the first mode is encoded in the preferred EDID DTD.
        pInArgs->pMonitorModes[i] = CreateIddCxMonitorMode(m.width, m.height, m.refresh,
...
NTSTATUS IddSampleMonitorGetDefaultModes(IDDCX_MONITOR MonitorObject, const IDARG_IN_GETDEFAULTDESCRIPTIONMODES* pInArgs, IDARG_OUT_GETDEFAULTDESCRIPTIONMODES* pOutArgs)
{
    ...
    pOutArgs->DefaultMonitorModeBufferOutputCount = SweetDisplay::ModeCount;
    pOutArgs->PreferredMonitorModeIdx = 0;
    const auto status = SweetDisplay::CheckBuffer(pInArgs->DefaultMonitorModeBufferInputCount, pInArgs->pDefaultMonitorModes);
    if (status != SweetDisplay::BufferStatus::Fill) return ModeBufferResult(status);
    for (DWORD i = 0; i < SweetDisplay::ModeCount; ++i)
    {
        const auto& m = SweetDisplay::Modes[i];
        pInArgs->pDefaultMonitorModes[i] = CreateIddCxMonitorMode(m.width, m.height, m.refresh);
    }
    return STATUS_SUCCESS;
}
...
{
    UNREFERENCED_PARAMETER(MonitorObject);
    pOutArgs->TargetModeBufferOutputCount = SweetDisplay::ModeCount;
    const auto status = SweetDisplay::CheckBuffer(pInArgs->TargetModeBufferInputCount, pInArgs->pTargetModes);
    if (status != SweetDisplay::BufferStatus::Fill) return ModeBufferResult(status);
    for (DWORD i = 0; i < SweetDisplay::ModeCount; ++i)
    {
        const auto& m = SweetDisplay::Modes[i];
        pInArgs->pTargetModes[i] = CreateIddCxTargetMode(m.width, m.height, m.refresh);
    }
    return STATUS_SUCCESS;
}
_Use_decl_annotations_
